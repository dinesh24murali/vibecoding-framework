...
| T004 | Migration runner + release execution command | in_progress | P0 | solo-dev | Command Pattern | Schemas: all persisted entities | `backend/cmd/migrate/main.go`, `backend/internal/migrate/runner.go` | `backend/Makefile` | T003 | `migrate up` runs ordered scripts with failure stop | integration | Release failures without safe migration |
