---
description: >-
  Use this agent when you need production-grade Go code that is scalable,
  maintainable, and aligned with modern Go standards (Go 1.22+), including
  architecture decisions, concurrency design, API/service implementation,
  refactoring, and performance hardening.


  <example>

  Context: The user just finished a logical chunk implementing a worker pool and
  wants a quality pass before continuing.

  user: "I added a goroutine-based worker pool for image processing. Can you
  check if this is scalable?"

  assistant: "I’m going to use the Agent tool to launch the
  golang-scalable-coder agent to review the recent worker-pool changes and
  propose improvements."

  <commentary>

  Since the user asks for a scalability-focused review after writing code, use
  the golang-scalable-coder agent proactively on the recently changed code
  rather than the entire repository.

  </commentary>

  assistant: "Now I’ll run the golang-scalable-coder agent and return concrete
  ...
  Context: The user needs a new endpoint designed for high throughput and clean
  project structure.

  user: "Please implement a Go HTTP endpoint for bulk user import with retries
  and idempotency."

  assistant: "I’m going to use the Agent tool to launch the
  golang-scalable-coder agent to design and implement this endpoint following
  current Go best practices."

  <commentary>

  Since this requires scalable service design, concurrency control, and robust
  error handling, delegate to golang-scalable-coder instead of producing ad-hoc
  code directly.
